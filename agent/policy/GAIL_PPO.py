...
class GAIL_PPO:
    def __init__(self, args):
        ...
        self.rl_critic_net = Critic(args, 'rl_critic').to(self.device)  # RL Critic
        self.expert_critic_net = Critic(args, 'expert_critic').to(self.device)  # Expert Critic
        ...
        # 优化器
        self.actor_optim = torch.optim.Adam(self.actor_net.parameters(), lr=args.lr_actor)
        self.rl_critic_optim = torch.optim.Adam(self.rl_critic_net.parameters(), lr=args.lr_critic)
        self.expert_critic_optim = torch.optim.Adam(self.expert_critic_net.parameters(), lr=args.lr_critic)
    ...
    def add_graph(self, obs, action, logger):
        ...
        wrapper = WrapperStateAction3(self.actor_net, self.rl_critic_net, self.discr_net)
    ...
    def train(self, transitions):
        ...
        with torch.no_grad():
            # 计算两个 Critic 网络的值
            ppo_value = self.rl_critic_net(agent_obs)
            ppo_next_value = self.rl_critic_net(agent_next_obs)
            expert_value = self.expert_critic_net(agent_obs)
            expert_next_value = self.expert_critic_net(agent_next_obs)

        # 3. 计算 GAE
        ppo_advantages = self.compute_gae(ppo_rewards, ppo_value.cpu().numpy(), ppo_next_value.cpu().numpy(),
                                          trans_done.cpu().numpy())
        expert_advantages = self.compute_gae(expert_rewards, expert_value.cpu().numpy(),
                                             expert_next_value.cpu().numpy(), trans_done.cpu().numpy())
        ppo_advantages = torch.tensor(ppo_advantages, dtype=torch.float32).reshape(ppo_value.shape).to(self.device)
        expert_advantages = torch.tensor(expert_advantages, dtype=torch.float32).reshape(expert_value.shape).to(
            self.device)
        ...
        for _ in range(self.update_nums):
            ...
            for batch_ids in batch_id_set:
                ...
                # RL Critic 更新
                ppo_critic_loss = (batch_ppo_returns - self.rl_critic_net(batch_obs)).pow(2).mean()
                self.rl_critic_optim.zero_grad()
                ppo_critic_loss.backward()
                torch.nn.utils.clip_grad_norm_(self.rl_critic_net.parameters(), self.max_grad_norm)
