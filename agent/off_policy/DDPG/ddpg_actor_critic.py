...
from agent.off_policy.offp_config import DDPG_CONFIG
...
class DeterministicActor(ChkptModule):
    def __init__(self, args, network_type):
        ...
        self.hidden_dim = DDPG_CONFIG["actor_hidden_dim"]
...
class DeterministicCritic(ChkptModule):
    def __init__(self, args, network_type):
        ...
        self.hidden_dim = DDPG_CONFIG["critic_hidden_dim"]
        self.output_dim = 1
...
class DeterministicActor2d(ChkptModule):
    def __init__(self, args, network_type):
        ...
        hidden_dim = DDPG_CONFIG["actor_hidden_dim"]
        self.fc1 = nn.Linear(conv_out_size, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, int(hidden_dim / 2))
        self.output = nn.Linear(int(hidden_dim / 2), args.agent_action_dim)
...
class DeterministicCritic2d(ChkptModule):
    def __init__(self, args, network_type):
        ...
        # Fully connected layers
        hidden_dim = DDPG_CONFIG["critic_hidden_dim"]
        self.fc1 = nn.Linear(conv_out_size, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, int(hidden_dim / 2))
